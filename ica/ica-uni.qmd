...
i think the most beneficial visualization of 'rating' would be a bar graph.

b.  What about a visualization of the *quantitative* `elevation` variable?

i think the most beneficial visualization of elevation would be a line graph, so you can see the fluctuations of the elevations.  
...
ggplot(hikes, aes(x = rating)) +
  geom_bar()
```
the change in this code added data onto the chart, making moderate with the most count at around 27, easy with 11, difficult with 7.
...
```
the change in this code added a y-axis name of 'Number of hikes' and it also made the r in 'rating' capital.
...
ggplot(hikes, aes(x = rating)) +
  ...
  labs(x = "Rating", y = "Number of hikes")
```
this code changed the bar color to blue.
...
ggplot(hikes, aes(x = rating)) +
  ...
  labs(x = "Rating", y = "Number of hikes")
```
this code caused the blue bars to have an orange outline.
...
ggplot(hikes, aes(x = rating)) +
  ...
  theme_minimal()
```
this code changed the background of the graph, changing the color from grey to white. this is due to the line 'theme_minimal()'
...
the purpose of the '+' is to add multiple commands at once (it looks more clean). we use it when we plan to add on to what we've already done otherwise, it will change the graph without old changes. 

-   We added the bars using `geom_bar()`? Why "geom"?

i am not sure as to why we use geom, but it kind of reminds me of geometry. so maybe we use it to determine that it is a shape?

-   What does `labs()` stand for?

i am not sure what labs stands for, but i see it as the chart information. so maybe it stands for 'labels'? 

-   What's the difference between `color` and `fill`?

fill completely turns the bar into the color chosen where as color is the outline of the shape.
